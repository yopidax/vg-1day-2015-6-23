...
class ImageHelper {
    ...
    */
    static func imageWithBase64EncodedString(base64EncodedString: String) -> UIImage? {
        if let imageData = NSData(base64EncodedString: base64EncodedString, options: nil) , image = UIImage(data: imageData) {
            return image
        }
        return nil
    }
    ...
    */
    static func base64encodedStringWithImage(image: UIImage) -> String {
        let jpegData = NSData(data: UIImageJPEGRepresentation(image, 1.0))
        let base64encodedString = jpegData.base64EncodedStringWithOptions(nil)
